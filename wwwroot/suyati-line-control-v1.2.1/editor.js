...
(function ($) {
    ...
    var methods = {
        ...
        imageWidget: function () {
            ...
            getImageURL.css("border-bottom", 0);
            var imageURL = $('<input/>', {
                type: "url",
                class: 'form-control',
                id: "imageURL",
            ...
            var getURL = $("<button/>", {
                ...
                image.onerror = function () {
                    ...
                    methods.showMessage.apply(this, ["imgErrMsg", "无效的图片链接"]);
                    return false;
                };
            ...
            var images = $("<button/>", {
                class: "btn btn-success",
                type: "button",
                // style: "padding: 0.4em 1.6em; font-size: 1.2em;",
                style: "padding: 0.4em 1.6em;",
                "data-toggle": "modal",
                "data-target": "#imagesNode"
            }).html("图库").click(getImages2);

            var files = $("<button/>", {
                class: "btn btn-success",
                type: "button",
                // style: "padding: 0.4em 1.6em; font-size: 1.2em;",
                style: "padding: 0.4em 1.6em; margin-left: 1em;",
                "data-toggle": "modal",
                "data-target": "#filesNode"
            }).html("文件库").click(getFiles2);

            imagesNodeBtn.onclick = function () {

                imagesNode.querySelectorAll("tbody input").forEach(function (input) {
                    ...
                    if (input.checked) {
                        ...
                        $(img).appendTo(a).click(function () {
                            $('#imageList').data('current', $(img).attr('src'));
                        });
            ...
            filesNodeBtn.onclick = function () {

                filesNode.querySelectorAll("tbody input").forEach(function (input) {
                    ...
                    if (input.checked) {
                        ...
                        div.dataset.src = input.value;
                        div.dataset.title = input.dataset.title;
                        div.innerText = input.dataset.title;

                        $(div).appendTo(a).click(function () {
                            $('#imageList').data('src', div.dataset.src);
                            $('#imageList').data('title', div.dataset.title);
                        });
            ...
            uploadImageBar.append(images);
            uploadImageBar.append(files);
        ...
        tableWidget: function (mode) {
            ...
            var tblCntr = $('<div/>', { //Outer Container Div
                ...
                id: "tblErrMsg" + idExtn
                ...
                }).append($('<label/>', {for: "tblRows" + idExtn, text: "行"}
                )).append($('<input/>', {
                    id: "tblRows" + idExtn,
                    type: "text",
                    class: "form-control form-control-width",
                    value: 2
                })).append($('<label/>', {for: "tblColumns" + idExtn, text: "列"}
                )).append($('<input/>', {
                    id: "tblColumns" + idExtn,
                    type: "text",
                    class: "form-control form-control-width",
                    value: 2
                })).append($('<label/>', {for: "tblWidth" + idExtn, text: "宽"}
                )).append($('<input/>', {
                    id: "tblWidth" + idExtn,
                    type: "text",
                    class: "form-control form-control-width",
                    value: 400
                })).append($('<label/>', {for: "tblHeight" + idExtn, text: "高"}
                )).append($('<input/>', {
                    id: "tblHeight" + idExtn,
                    type: "text",
                    class: "form-control form-control-width",
                    ...
                    class: "col-xs-12 col-sm-6 col-md-6 col-lg-6"
                }).append($('<label/>', {for: "tblAlign" + idExtn, text: "对齐"}
                )).append($('<select/>', {id: "tblAlign" + idExtn, class: "form-control form-control-width"}
                ).append($('<option/>', {text: "选择", value: ""}
                )).append($('<option/>', {text: "靠左", value: "left"}
                )).append($('<option/>', {text: "居中", value: "center"}
                )).append($('<option/>', {text: "靠右", value: "right"}))
            ...
            var menuItems = {
                ...
                'insert_link': {
                    ...
                    "onSave": function () {
                        ...
                        if (targetURL == '') {
                            methods.showMessage.apply(editorObj, ["errMsg", "请输入地址"]);
                            return false;
                        }
                        if (!targetURL.match(urlPattern)) {
                            methods.showMessage.apply(editorObj, ["errMsg", "输入地址无效"]);
                            return false;
                        }
                        if (range == '' && targetText == '') {
                ...
                'insert_img': {
                    ...
                    "onSave": function () {
                        ...
                        } else if ($('#imageList').data('src')) {
                            var src = $('#imageList').data('src');
                            var imageStr = '<a href="' + src
                                + '" download="' + $('#imageList').data('title') + src.substr(src.lastIndexOf(".")) + '">'
                                + $('#imageList').data('title') + '</a>';
                            if (navigator.userAgent.match(/MSIE/i) || navigator.userAgent.match(/Windows NT.*Trident\//)) {
                                methods.restoreSelection.apply(this, [imageStr, 'html'])
                            } else {
                                document.execCommand('insertHTML', false, imageStr);
                            }
                        } else {
                            methods.showMessage.apply(this, ["imgErrMsg", "请单击要选择的图片"]);
                            return false;
                        }
